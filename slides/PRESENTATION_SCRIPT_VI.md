...
Script này được soạn cho 33 slide chính. Các slide 34-40 thuộc `Appendix` và chỉ sử dụng khi cần giải thích thêm trong phần hỏi đáp. Nội dung tiếng Việt được viết theo văn phong nói học thuật; các thuật ngữ chuyên ngành, tên phương pháp, model, dataset và metric được giữ nguyên bằng tiếng Anh.
...
## Slide 29 - Local Experiment 1: End-Task Evaluation
...
Theo rule-based final-answer evaluation, Plain đúng 9 trên 10 case, tương đương 90 phần trăm. MedRaC đúng 10 trên 10 case, tương đương 100 phần trăm.

Hai method sử dụng cùng generator `gpt-4.1`. Kết quả cho thấy MedRaC đúng thêm một case, tương ứng chênh lệch 10 percentage points trên subset này. Tuy nhiên, với `n = 10`, đây chỉ là descriptive result và không hỗ trợ statistical inference.

## Slide 30 - Local Experiment 2: Step-wise Evaluation

Slide này chuyển từ end-task accuracy sang process-level evaluation trên chính các stored outputs của experiment.

Đối với Plain, Formula đúng 10 trên 10, Extraction 9 trên 10, Calculation 10 trên 10, Answer 8 trên 10 và Strict Correctness 8 trên 10. Đối với MedRaC, các kết quả tương ứng là 10, 10, 10, 9 và 9 trên 10.

Formula và Calculation đều giữ nguyên 10 trên 10 ở cả hai method. Cải thiện nằm tại Extraction, Answer và Strict Correctness, mỗi metric tăng một case.

Các giá trị Conditional Correctness và provenance đầy đủ được chuyển xuống Appendix để slide chính tập trung vào kết luận process-level.

## Slide 31 - Scope, Limitations, and Evaluation Caveats
...
Về local experiment, quy mô chỉ có 10 fixed case, sử dụng model substitution qua GitHub Models và một retrieval index riêng gồm 55 formula. Step-wise result được tạo post-hoc bằng LLM Judge trên stored outputs, không phải human annotation.

Artifact cũng ghi nhận một timeout repair và một Answer adjudication theo benchmark acceptance interval. Các thay đổi này được công khai để bảo đảm provenance, nhưng đồng thời cho thấy kết quả không phải paper-exact reproduction.
...
## Slide 33 - Questions and Discussion
...
## Slide 34 - Appendix: Complete Main-Results Table
...
## Slide 35 - Appendix: Complete Metric Definitions
...
## Slide 36 - Appendix: Paper Discrepancies
...
## Slide 37 - Appendix: Formula-Bank Scaling
...
## Slide 38 - Appendix: Local Experiment Provenance

Slide này tổng hợp provenance của hai artifact. Generation run sử dụng 10 case, gồm 7 equation-based và 3 rule-based case; generator là `gpt-4.1`, temperature bằng 0 và seed bằng 42. Không có failed hoặc skipped sample sau documented timeout repair.

Post-hoc evaluation sử dụng `gpt-4.1` trên 20 method-sample unit. Mỗi unit có bốn judge call tương ứng Formula, Extraction, Calculation và Answer. Strict Correctness, Conditional Correctness và First Error Attribution được tổng hợp từ các verdict này.

Có hai amendment cần công khai. Thứ nhất, stored MedRaC code của một case được thực thi lại với timeout năm giây sau khi timeout hai giây; không có API request nào được gọi lại. Thứ hai, một Answer verdict được adjudicate theo benchmark acceptance interval; original verdict vẫn được giữ trong provenance.

## Slide 39 - Appendix: Safe-Execution Boundary
